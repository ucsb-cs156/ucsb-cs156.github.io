...
Then it's time to check the database. No need to fire up the app at first; we can check it directly 
...
Now, try the app itself by visiting the url (e.g. <https://organic-qa.dokku-00.cs.ucsb.edu>).  At the end of the day, if the database
tables look perfect, but the app is broken, that's not good, so be sure to check that too.
